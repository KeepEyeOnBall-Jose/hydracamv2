# HydraCam Status Archive (2026)

This file holds the historical, dated content moved out of
`docs/control/status-and-roadmap.md` on 2026-08-26 so that document could stay
under its line-count target. Nothing here is deleted — every section below is
verbatim from the prior `status-and-roadmap.md` (as of the 2026-07-14 refresh)
and is preserved for evidence provenance and audit trail.

Do not treat any dated entry below as current. For current status, roadmap,
and priorities, see `docs/control/status-and-roadmap.md`. For raw evidence
packs, see `logs/verification-runs/`.
